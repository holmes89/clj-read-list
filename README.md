...
* [`src/clj/reading_list/`](src/clj/reading_list/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/cljs/reading_list/`](src/cljs/reading_list/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/reading_list/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `reading-list` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/reading_list/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reading-list.example
  (:require [reading-list.config :as config])
...
```sh
port=2000 java -jar target/reading-list.jar
```
